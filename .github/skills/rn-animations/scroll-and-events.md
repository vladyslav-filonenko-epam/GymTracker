...
> **GymTracker note:** This project uses `FlashList` from `@shopify/flash-list` instead of `FlatList`. `useScrollOffset` works with `FlashList`. For `useAnimatedScrollHandler`, wrap `FlashList` in an `Animated.createAnimatedComponent(FlashList)` or use the `onScroll` prop directly.
