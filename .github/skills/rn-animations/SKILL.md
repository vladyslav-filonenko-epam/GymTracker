...
This project uses `react-native-reanimated ^4.5.3` with the New Architecture (Fabric + TurboModules) enabled.

Use `FlashList` from `@shopify/flash-list` for all lists in this project — never `FlatList`.
...
- **NEVER use `runOnUI`**. It is replaced by `scheduleOnUI(fn, ...args)` from `react-native-worklets`.
